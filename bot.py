...
async def clock_cmd(update,ctx):
    uid=update.effective_user.id; inc(uid,True); await send_sticker(update,'tools'); raw=update.message.text[len('/clock'):].strip()
    if not raw: await update.message.reply_text('/clock 10m Uống nước\n/clock 1h Nghỉ\n/clock 14:30 Học bài'); return
    p=raw.split(maxsplit=1); trigger=p[0]; text=p[1] if len(p)>1 else 'Báo thức!'; now=datetime.now(timezone.utc); fire=None
    m=DUR.match(trigger)
    if m:
        fire=now+timedelta(seconds=int(m.group(1))*{'s':1,'m':60,'h':3600,'d':86400}[m.group(2).lower()])
    else:
        m=CLOCK.match(trigger)
        if not m: await update.message.reply_text('/clock 10m Nội dung\n/clock 14:30 Nội dung'); return
        h,mi=int(m.group(1)),int(m.group(2));
        if h>23 or mi>59: await update.message.reply_text('❌ Invalid time'); return
        row=get_user(uid); tz=ZoneInfo(row[4]) if row and row[4] else ZoneInfo(DEFAULT_TZ); local=datetime.now(tz); target=local.replace(hour=h,minute=mi,second=0,microsecond=0); target+=timedelta(days=1) if target<=local else timedelta(0); fire=target.astimezone(timezone.utc)
    ...
    row=get_user(uid); tz=ZoneInfo(row[4]) if row and row[4] else ZoneInfo(DEFAULT_TZ); local=fire.astimezone(tz)
    await update.message.reply_text(f'⏰ {local.strftime("%d/%m %H:%M")}\n📝 {text}\n🆔 {alarm_id}')
